...
**微信小程序（管理层速查版，2026-09-08 纳入架构）**：独立于上述 Web 路由树，面向"领导手机上看数"，仅四块只读能力——

| 小程序页面 | 内容 | 数据来源 |
|---|---|---|
| 首页指标卡 | 多项目核心指标（利润率/产值/进度/预警数）＋周排名 | 网关现有 API 复用 |
| 单项目速查 | KPI 摘要＋现金流简表＋里程碑进度条（只读） | 网关现有 API 复用 |
| AI 问答 | 纯文本问答（SSE 流式，无附件/推演面板），引用徽章简化版 | `/api/v1/agent-chat`（SSE chunked 小程序端手写适配） |
| 登录 | 微信授权登录换取网关 HMAC 令牌（登录态映射），RBAC 复用服务端拦截 | 网关 `/api/auth/*` |

技术形态：uni-app/Taro 单代码库（可扩展支付宝/企业微信端）；桌面端重交互页（成本测算编辑、甘特图、推演、文档管理）**不进小程序**。前置条件：网关公网 HTTPS（域名＋备案＋SSL），暴露面需负责人批准——详见《成本估算》§6.5。
